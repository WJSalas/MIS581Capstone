...
## Convert decimal degrees (DD) to radians
...
#     long1 <- deg2rad(Well_Data$long1[i])
#     lat1 <- deg2rad(Well_Data$lat1[i])
#     long2 <- deg2rad(Assessor_Data$long2[j])
#     lat2 <- deg2rad(Assessor_Data$lat2[j])
#     
#     # Calculate the distances between the Well and home
#     dist_km <- gcd.hf(long1, lat1, long2, lat2)
#     
#     print(Assessor_Data$ACCOUNTNO[j])
#     print(Well_Data$API_Label[i])
#     print(dist_km)
#   }
# }
# 
# # Rename the columns in the df Output
# #colnames(df) <- c("API", "strap", "Dist_to_Home")
